...
class Sizes:
    ...
    def set_size_case(self, w_screen: int, h_screen: int) -> None:
        ...
        diagonal_case_from_w = w_screen * 3 / 4 / ratio_w_screen
        diagonal_case_from_h = h_screen * 3 / 4 / ratio_h_screen

        if diagonal_case_from_w < diagonal_case_from_h:
            self.w_case = Math.floor(diagonal_case_from_w * ratio_w_case)
            self.h_case = Math.floor(self.w_case * ratio_h_case / ratio_w_case)
        else:
            self.h_case = Math.floor(diagonal_case_from_h * ratio_h_case)
            self.w_case = Math.floor(self.h_case * ratio_w_case / ratio_h_case)

        if 22 * self.h_case > Sizes.h_max_window:
            diagonal_case_from_h = Sizes.h_max_window / ratio_h_screen
            self.h_case = Math.floor(diagonal_case_from_h * ratio_h_case)
            self.w_case = Math.floor(self.h_case * ratio_w_case / ratio_h_case)
        else:
            pass
